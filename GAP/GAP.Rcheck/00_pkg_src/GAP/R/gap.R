...
#'   }
#' @param alpha A numeric value for the alpha parameter in the GAP model
#' @param p_threshold A numeric value for the p-value threshold to define significant SNPs.
#'   `p_thershold` is still accepted for backward compatibility.
#' @param p_thershold Deprecated spelling retained for backward compatibility.
...
#'
#' @export
GAP_bayesian_prior <- function(input, alpha, p_threshold = NULL, random, p_thershold = NULL) {
  if (is.null(p_threshold)) {
    p_threshold <- p_thershold
  }

  if (is.null(p_threshold)) {
    stop("`p_threshold` must be supplied.")
  }
